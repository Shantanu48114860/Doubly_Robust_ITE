...
class Generator(nn.Module):
    ...
    def forward(self, y):
        y = F.relu(self.bn1(self.shared1(y)))
        y = F.relu(self.bn1(self.shared2(y)))

        y0 = F.relu(self.bn1(self.hidden_y0(y)))
        y0 = self.out_y0(y0)

        y1 = F.relu(self.bn1(self.hidden_y1(y)))
        y1 = self.out_y1(y1)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x, y0, y1):
        ...
        x_t = F.leaky_relu(self.bn1(self.hidden1(x_t)), 0.1)
        x_t = F.leaky_relu(self.bn1(self.hidden2(x_t)), 0.1)
...
class QHead(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.bn1(self.hidden1(x)), 0.1)
        x = F.leaky_relu(self.bn1(self.hidden2(x)), 0.1)
