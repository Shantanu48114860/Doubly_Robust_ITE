...
# treated_np = np.array(wo_DR_treated_DR_prob, treated_DR_prob)
# control_np = np.array(control_DR_prob, wo_DR_control_DR_prob)

DR_Probs_np = np.array([wo_DR_treated_DR_prob, control_DR_prob])
wo_DR_Probs_np = np.array([treated_DR_prob, wo_DR_control_DR_prob])
...
# plt.show()
plt.draw()
plt.savefig("Plots/Jobs_BoxPlots_y_f.jpeg", dpi=220)
plt.clf()
